//
//  PMSSPushBackEndRegistrationRequestDataSpec.mm
//  PMSSPushSDK
//
...
#import "PMSSPushBackEndRegistrationRequestDataTest.h"
#import "NSObject+PMSSJsonizable.h"
#import "PMSSPushErrors.h"

SPEC_BEGIN(PMSSPushBackEndRegistrationRequestDataSpec)

describe(@"PMSSPushBackEndRegistrationRequestData", ^{
    
    __block PMSSPushRegistrationRequestData *model;
    ...
    it(@"should be initializable", ^{
        model = [[PMSSPushRegistrationRequestData alloc] init];
    ...
    context(@"deserialization", ^{
        ...
        it(@"should handle a nil input", ^{
            NSError *error;
            model = [PMSSPushRegistrationRequestData pmss_fromJSONData:nil error:&error];
            [[model  should] beNil];
            [[error shouldNot] beNil];
            [[error.domain should] equal:PMSSPushErrorDomain];
            [[theValue(error.code) should] equal:theValue(PMSSPushBackEndRegistrationDataUnparseable)];
        });
        
        it(@"should handle empty input", ^{
            NSError *error;
            model = [PMSSPushRegistrationRequestData pmss_fromJSONData:[NSData data] error:&error];
            [[model should] beNil];
            [[error shouldNot] beNil];
            [[error.domain should] equal:PMSSPushErrorDomain];
            [[theValue(error.code) should] equal:theValue(PMSSPushBackEndRegistrationDataUnparseable)];
        ...
        it(@"should handle bad JSON", ^{
            ...
            model = [PMSSPushRegistrationRequestData pmss_fromJSONData:JSONData error:&error];
            [[model  should] beNil];
            [[error shouldNot] beNil];
        });
        ...
        it(@"should construct a complete request object", ^{
            ...
            model = [PMSSPushRegistrationRequestData pmss_fromJSONData:data error:&error];
            [[error should] beNil];
            [[model.os should] equal:TEST_OS];
    ...
    context(@"serialization", ^{
        ...
        context(@"populated object", ^{
            ...
            it(@"should be dictionaryizable", ^{
                dict = [model pmss_toFoundationType];
            });
            
            it(@"should be JSONizable", ^{
                NSData *JSONData = [model pmss_toJSONData:nil];
