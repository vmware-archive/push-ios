//
//  PMSSRegistrationParametersSpec.mm
//  PMSSSDK
//
...
SPEC_BEGIN(PMSSRegistrationParametersSpec)

void (^checkPramatersAreValid)(NSString *, PMSSParameters *) = ^(NSString *pramType, PMSSParameters *model) {
    NSDictionary *properties = [PMSSClassPropertyUtility propertiesForClass:[PMSSParameters class]];
    [properties enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *propertyType, BOOL *stop) {
        ...
        if ([propertyName rangeOfString:pramType options:NSCaseInsensitiveSearch].length > 0 && propertyType.length > 1) {
            ...
            BOOL (^valid)(PMSSParameters *) = ^BOOL(PMSSParameters *model) {
...
describe(@"PMSSRegistrationParameters", ^{
    
    __block PMSSParameters *model;
    ...
    context(@"initializing with bad arguments programatically", ^{
        ...
        beforeEach(^{
            model = [PMSSParameters parameters];
    ...
    context(@"initializing with valid arguments from plist", ^{
        ...
        beforeEach(^{
            model = [PMSSParameters parametersWithContentsOfFile:[[NSBundle bundleForClass:[self class]] pathForResource:@"PMSSParameters-Valid" ofType:@"plist"]];
        });
    ...
    context(@"initializing with invalid arguments from plist", ^{
        ...
        beforeEach(^{
            model = [PMSSParameters parametersWithContentsOfFile:@"PMSSParameters-Invalid.plist"];
        });
