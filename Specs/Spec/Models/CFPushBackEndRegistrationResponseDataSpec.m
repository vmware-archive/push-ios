//
//  CFPushBackEndRegistrationResponseDataSpec.mm
//  CFPushSDK
//
...
#import "CFPushBackEndRegistrationDataTest.h"
#import "CFPushBackEndRegistrationResponseDataTest.h"
#import "CFPushErrors.h"


SPEC_BEGIN(CFPushBackEndRegistrationResponseDataSpec)

describe(@"CFPushBackEndRegistrationResponseData", ^{
    
    __block CFPushRegistrationResponseData *model;
    ...
    it(@"should be initializable", ^{
        model = [[CFPushRegistrationResponseData alloc] init];
    ...
    context(@"deserialization", ^{
        ...
        it(@"should handle a nil input", ^{
            NSError *error;
            model = [CFPushRegistrationResponseData fromJSONData:nil error:&error];
            [[model should] beNil];
            [[error shouldNot] beNil];
            [[error.domain should] equal:CFPushErrorDomain];
            [[theValue(error.code) should] equal:theValue(CFPushBackEndRegistrationDataUnparseable)];
        });
        
        it(@"should handle empty input", ^{
            NSError *error;
            model = [CFPushRegistrationResponseData fromJSONData:[NSData data] error:&error];
            [[model should] beNil];
            [[error shouldNot] beNil];
            [[error.domain should] equal:CFPushErrorDomain];
            [[theValue(error.code) should] equal:theValue(CFPushBackEndRegistrationDataUnparseable)];
        ...
        it(@"should handle bad JSON", ^{
            ...
            model = [CFPushRegistrationResponseData fromJSONData:JSONData error:&error];
            [[model should] beNil];
            [[error shouldNot] beNil];
        });
        ...
        it(@"should construct a complete response object", ^{
            ...
            model = [CFPushRegistrationResponseData fromJSONData:data error:&error];
            [[error should] beNil];
            [[model.os should] equal:TEST_OS];
