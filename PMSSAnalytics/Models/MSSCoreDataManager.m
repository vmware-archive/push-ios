...
static NSString *const kDatabaseDirectoryName = @"MSSPushAnalyticsDB";
static NSString *const kDatabaseFileName = @"MSSPushAnalyticsDB.sqlite";

@interface MSSCoreDataManager ()
...
static MSSCoreDataManager *_sharedCoreDataManager;
static dispatch_once_t onceToken;

@implementation MSSCoreDataManager
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:self.databaseURL options:nil error:&error]) {
        MSSPushCriticalLog(@"Error adding persistent store: %@, %@", error, [error userInfo]);
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSString *entityName = NSStringFromClass([MSSAnalyticEvent class]);
    ...
    [eventDataDescription setValueTransformerName:NSStringFromClass([MSSJSONValueTransformer class])];
...
- (NSURL *)databaseURL
{
    ...
    if (![fileManager fileExistsAtPath:[directoryURL path]]) {
        ...
        if (![fileManager createDirectoryAtURL:directoryURL withIntermediateDirectories:YES attributes:nil error:&error]) {
            MSSPushCriticalLog(@"Error creating database directory %@: %@", [directoryURL lastPathComponent], error);
            
        } else {
            NSError *error = nil;
            if (![directoryURL setResourceValue:@YES forKey:NSURLIsExcludedFromBackupKey error:&error]) {
                MSSPushCriticalLog(@"Error excluding %@ from backup %@", [directoryURL lastPathComponent], error);
            }
        }
    }
...
- (NSArray *)managedObjectsWithEntityName:(NSString *)entityName
{
    ...
    [self.managedObjectContext performBlockAndWait:^{
        ...
        if ([NSClassFromString(entityName) conformsToProtocol:@protocol(MSSSortDescriptors)]) {
