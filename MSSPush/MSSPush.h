 ...
 *
 * Note that on iOS 8.0+ you will still need to call [MSSPush registerforPushNotifications] in order to register
 ...
 *
 * Before calling this method, you should call [MSSPush setRegistrationParameters] to provide the registration
 * parameters and call [MSSPush setRemoteNotificationTypes] if you want to provide a different subset of the
 * notification types.  If you want a callback indicating success or failure of the registration operation then
 * you should also call [MSSPush setCompletionBlockWithSuccess:failure].
 *
 * If you want to do automatic registration, then instead provide a PLIST file called "MSSParameters.plist" with
