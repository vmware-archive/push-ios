...
@implementation PMSSAppDelegateProxy
...
- (void)fetchCompletionHandler:(UIBackgroundFetchResult)result original:(CompletionHandler)originalCompletionHandler
{
    @synchronized(self) {
        ...
        fetchResult = [PMSSAppDelegateProxy mergeNewBackgroundFetchResult:result withOldFetchResult:fetchResult];
...
- (void)forwardInvocation:(NSInvocation *)invocation
{
    if (!self.originalAppDelegate) {
        [NSException raise:@"PMSSNilAppDelegate" format:@"PMSSAppDelegateProxy originalApplicationDelegate was nil."];
    }
